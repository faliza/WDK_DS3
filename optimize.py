...
def optimize_acqf_binary_local_search(acqf, afo_config, pareto_points: torch.Tensor, q: int = 1):
    ...
    for _ in range(q):
        ...
        #----------------Constraunt to pich at least 3--------------------
        x_init_candts = torch.zeros((n_initial_candts, n_binary), **tkwargs)
        for i in range(n_initial_candts):
            ones_indices = torch.randperm(n_binary)[:torch.randint(3, n_binary, (1,))]
            x_init_candts[i, ones_indices] = 1
        #---------------------------
