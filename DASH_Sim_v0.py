...
def run_simulator(scale_values=common.scale_values_list):
    ...
    print(f"--------------------scheduler: {common.scheduler}")
    ...
    print(common.str_to_list(config['DEFAULT']['job_file']))
    ...
    if (common.simulation_mode == 'validation'):
        ...
        for i,resource in enumerate(resource_matrix.list):
            ...
            new_PE = processing_element.PE(env, resource.type, resource.name,
                                           resource.ID, resource.cluster_ID, resource.capacity,resource.throughput,resource.power) # Generate a new PE with this generic process
            DASH_resources.append(new_PE)
        # end for
        # print(f"DASH_resources {DASH_resources}")
        # Construct the scheduler
        DASH_scheduler = scheduler.Scheduler(
            env,
            resource_matrix,
            common.scheduler,
            DASH_resources,
        ...
        #     assigned_pe = DASH_resources[task.PE_ID]
        #     print("%-30s | %-10s | %-20s | %-10.6f" % (
        #         f"{task.ID}, {task.name}",
        #         assigned_pe.ID,
        #         assigned_pe.name,
        #         assigned_pe.power
        ...
        for (ind,scale) in enumerate(common.scale_values_list):
            ...
            for iteration in range(common.num_of_iterations):                       # Repeat the simulation for a given number of numbers for each lambda value
                ...
                for i,resource in enumerate(resource_matrix.list):
                    # Define the PEs (resources) in simpy environment
                    new_PE = processing_element.PE(env, resource.type, resource.name,
                                                   resource.ID, resource.cluster_ID, resource.capacity,resource.throughput) # Generate a new PE with this generic process
                    DASH_resources.append(new_PE)
                # end for
                ...
                job_gen = job_generator.JobGenerator(env, resource_matrix, jobs, DASH_scheduler, DASH_resources)
                print("Before creating SimulationManager")
                sim_core = DASH_Sim_core.SimulationManager(env, sim_done, job_gen, DASH_scheduler, DASH_resources,
                                                           jobs, resource_matrix)
                print("After creating SimulationManager")
                
                print("Before env.run")
