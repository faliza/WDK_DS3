...
def sensor_sleep():
    ...
    time.sleep(0.1)
    sensor_status = read_register(BMI160_I2C_ADDR, 0x03, 1)[0]  # Read acc
    print("status:", sensor_status)
    acc_mode  = (sensor_status >> 4) & 0x03
    gyro_mode = (sensor_status >> 2) & 0x03

    print("ACC mode:", acc_mode)
    print("GYRO mode:", gyro_mode)
    return
def sensor_on():
    ...
    write_register(BMI160_I2C_ADDR, 0x7E, 0x15)  # GYR_NORMAL_MODE
    time.sleep(0.1)
    sensor_status = read_register(BMI160_I2C_ADDR, 0x03, 1)[0]
    print("status:", sensor_status)
    acc_mode  = (sensor_status >> 4) & 0x03
    gyro_mode = (sensor_status >> 2) & 0x03
...
for w in range(0, num_windows):
    ...
    if exit_times[w]< window_length:
            sensor_sleep()
            time.sleep(window_length-exit_times[w])
