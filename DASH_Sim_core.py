...
#     """
#     Safe version of deepcopy used in DS3.
#     Avoids recursion on objects with self-references.
#     """
#     try:
#         # Skip deepcopy for known recursive DS3 classes
#         if hasattr(obj, "__class__") and obj.__class__.__name__ in [
#             "ProcessingElement", "Task", "Job", "Scheduler"
#         ]:
#             print("[SafeCopy] Shallow copying:", obj.__class__.__name__)
#             return obj  # return reference — avoids recursion loops
#         else:
#             return original_deepcopy(obj, memo)
#     except Exception as e:
#         print("[SafeCopy] Deepcopy failed for", type(obj), ":", e)
#         return obj
...
class SimulationManager:
        ...
        for i, outstanding_task in enumerate(common.TaskQueues.outstanding.list):                       # Go over each outstanding task
            if (completed_task.ID in outstanding_task.predecessors):                                    # if the completed task is one of the predecessors
                ...
                if (common.shared_memory):
                    ...
                    if (common.DEBUG_SIM):
                        print('[D] Time %.6f: Data from task %d for task %d will be sent to memory in %d us'
            ...
            if (no_predecessors and not(currently_running) and not_in_ready_queue):
                if (common.PE_to_PE):                                                              # if PE to PE communication is utilized
                    ...
                    if (common.INFO_SIM) and (common.shared_memory):
                            print('[I] Time %.6f: Task %d ready times due to memory communication of its predecessors are'
                ...
                if ready_task.base_ID == task.ID:
                    ...
                    for predecessor in task.predecessors:
                        ...
                        if (common.PE_to_PE):
                            ...
                            if (common.DEBUG_SIM):
                                print('[D] Time %.6f: Data transfer from PE-%s to PE-%s for task %d from task %d is completed at %d us'
                        ...
                        if (common.shared_memory):
                            ...
                            if (common.DEBUG_SIM):
                                print('[D] Time %.6f: Data from memory for task %d from task %d will be sent to PE-%s in %d us'
                    ...
                    if (common.INFO_SIM) and (common.PE_to_PE):
                        print('[I] Time %.6f: Task %d execution ready times due to communication between PEs are'
                              %(self.env.now, ready_task.ID))
                        # print("hereeeee")
                        print('%12s'%(''), ready_task.PE_to_PE_wait_time)

                    if (common.INFO_SIM) and (common.shared_memory):
                        print('[I] Time %.6f: Task %d execution ready time(s) due to communication between memory and PE-%s are'
    ...
    def run(self):
        ...
        This function takes the next ready tasks and run on the spescific PE and update the common.TaskQueues.ready list accordingly.
        '''
        # ------------DTPM disabling
        ...
        while (True):                                                           # Continue till the end of the simulation
            ...
            if (not len(common.TaskQueues.ready.list) == 0):
                ...
                elif self.scheduler.name == 'MET_leastLatency':
                    self.scheduler.MET_leastLatency(common.TaskQueues.ready.list)
                elif self.scheduler.name == 'MET_capacity_aware':
                    self.scheduler.MET_capacity_aware(common.TaskQueues.ready.list) 
                elif self.scheduler.name == 'MET_LeastEnergy':
                    self.scheduler.MET_LeastEnergy(common.TaskQueues.ready.list)        
                ...
                elif self.scheduler.name == 'BO_TotalThroughput':    
                    self.scheduler.BO_TotalThroughput(common.TaskQueues.ready.list)
                else:
            ...
            if len(common.TaskQueues.executable.list) != 0:
                ...
                # print(f"executable_task.PE_ID; {executable_task.PE_ID}")
                for i, executable_task in enumerate(common.TaskQueues.executable.list):
                    # print("2. for PE blocking data collectione")
                    # print(f" executable_task.id3: {executable_task.ID}")
                    ...
                    # print(f" task_has_assignment: {task_has_assignment}")
                    ...
                    # print(f"PE {executable_task.PE_ID} capacity: {self.PEs[executable_task.PE_ID].capacity}, queue len: {len(self.PEs[executable_task.PE_ID].queue)}")
                    # print(f"{is_time_to_execute}, {PE_has_capacity}, {dynamic_dependencies_met} ,{task_has_assignment}")
